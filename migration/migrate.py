...
def run_migration(source_engine: Engine, target_engine: Engine, tables: set, batch_size: int = 100) -> list:
    """
    ...
    fk_graph = get_fk_graph(source_engine)
    table_order = resolve_table_order(fk_graph, tables)

    set_fk_checks(target_engine, enabled=False)
    ...
    for table_name in table_order:
        try:
            stats = migrate_table(table_name, source_engine, target_engine, batch_size)
            results.append(stats)
        except Exception as error:
            failed.append({"table": table_name, "error": str(error)})
            print(f"{table_name} failed: {error}")

    set_fk_checks(target_engine, enabled=True)
